...
router.get('/', function (req, res) {
    ...
    pool.query('SELECT serial_number FROM meters GROUP BY serial_number LIMIT 1000', function (err, result) {
        //console.log(result.rows[0].meter_number); // output: foo
        var metersList = [];
        result.rows.forEach(function (item, i, rows) {
            metersList.push(item.serial_number)
        });
        res.render('meter', { comment: 'You are my nigga, nigga', values: metersList });
